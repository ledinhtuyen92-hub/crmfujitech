...
7. QUY TẮC GỬI ẢNH (BẮT BUỘC):
- CHỈ GỬI ẢNH CỦA ĐÚNG PHẦN KIẾN THỨC ĐƯỢC DÙNG ĐỂ TRẢ LỜI: Bất kể hình ảnh (định dạng ![...](url)) xuất hiện trong [TRÍCH XUẤT KIẾN THỨC NỘI BỘ] hay [HÌNH ẢNH ĐÍNH KÈM - BẮT BUỘC GỬI CHO KHÁCH], bạn CHỈ ĐƯỢC PHÉP lấy ảnh thuộc về chính đoạn kiến thức (Q&A) mà bạn quyết định sử dụng để trả lời câu hỏi của khách hàng. Tuyệt đối KHÔNG copy ảnh từ các đoạn kiến thức khác mà bạn không dùng.
- PHẢI SAO CHÉP Y NGUYÊN TOÀN BỘ ẢNH CỦA ĐOẠN ĐÓ: Đối với đoạn kiến thức mà bạn đã chọn để trả lời, BẠN BẮT BUỘC PHẢI SAO CHÉP Y NGUYÊN TẤT CẢ các đường link URL ảnh nằm trong đoạn đó vào mảng 'image_urls' trong JSON. Tuyệt đối không được tự ý chọn lọc hay bỏ bớt bất kỳ ảnh nào có trong context của câu trả lời đó (Ví dụ: nếu đoạn Q&A bạn dùng có 5 ảnh thì BẮT BUỘC phải gửi đủ cả 5).
- TUYỆT ĐỐI KHÔNG tự ý gửi ảnh khi context KHÔNG có ảnh nào, hoặc khi tin nhắn của khách hoàn toàn không liên quan đến nội dung có ảnh (ví dụ: khách chỉ để lại số điện thoại, chào hỏi xã giao).
...
def call_gemini(api_key, agent, system_prompt, conversation_history):
    ...
    # Xử lý trường hợp Gemini trả về rỗng (bị safety filter hoặc quá tải)
    if not raw_text:
